...
REWARD_WEIGHTS = [0.15, 0.25, 0.20, 0.20, 0.10, 0.10]   # bin, exact, error, desc, len, format

SYSTEM_PROMPT = (
    "You are an expert in evaluating English to Malayalam machine translations. Your task is to provide a comprehensive evaluation including a quality score, error categorization, and a detailed analysis.\n\n"
        ...
        for _, r in dataframe.iterrows():
            prompt = [
                ...
                {"role": "user", "content": (
                    ...
                    f"Hypothesis (Malayalam): {r['Target']}\n\n"
                    "Read the weak human annotations for translated sentences and produce your detailed reasoning, error_type, description and DA score.")},
                {"role": "assistant", "content": r["merged_comment"]},
            ]
            rows.append({
                "prompt"    : prompt,
        ...
        if pd and gold_desc:
            try:
                ...
                _, _, f1 = bertscore.compute(predictions=[pd], 
                                             ...
                                             lang="en", 
                                             verbose=False,
                                             model_type="roberta-base-uncased")
                desc_r = f1.item()
            except Exception:
